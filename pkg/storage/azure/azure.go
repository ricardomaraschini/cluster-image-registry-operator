...
func (d *driver) CreateStorage(cr *imageregistryv1.Config) error {
	...
	if err != nil {
		util.UpdateCondition(cr, defaults.StorageExists, operatorapiv1.ConditionUnknown, storageExistsReasonConfigError, fmt.Sprintf("Unable to get infrastructure: %s", err))
		return err
	}
	...
	if err != nil {
		util.UpdateCondition(cr, defaults.StorageExists, operatorapiv1.ConditionUnknown, storageExistsReasonAzureError, fmt.Sprintf("Unable to process storage account: %s", err))
		return err
	}
	...
	if err != nil {
		util.UpdateCondition(cr, defaults.StorageExists, operatorapiv1.ConditionUnknown, storageExistsReasonAzureError, fmt.Sprintf("Unable to process storage container: %s", err))
		return err
	}
	...
	// We only set the storage management state to Managed if we created both the container
	// and the storage account, we set it to unmanaged only if it is not already set.
	if cr.Spec.StorageManagementState == "" && storageAccountCreated && containerCreated {
		cr.Spec.StorageManagementState = imageregistryv1.StorageManagementStateManaged
	} else if cr.Spec.StorageManagementState == "" {
		cr.Spec.StorageManagementState = imageregistryv1.StorageManagementStateUnmanaged
	}
