...
type Config struct {
	...
	// +optional
	Status ImageRegistryStatus `json:"status,omitempty" protobuf:"bytes,3,opt,name=status"`
}
...
type ImageRegistrySpec struct {
	...
	// +optional
	HTTPSecret string `json:"httpSecret,omitempty" protobuf:"bytes,2,opt,name=httpSecret"`
	// proxy defines the proxy to be used when calling master api, upstream
	// registries, etc.
	// +optional
	Proxy ImageRegistryConfigProxy `json:"proxy,omitempty" protobuf:"bytes,3,opt,name=proxy"`
	// storage details for configuring registry storage, e.g. S3 bucket
	// coordinates.
	// +optional
	Storage ImageRegistryConfigStorage `json:"storage,omitempty" protobuf:"bytes,4,opt,name=storage"`
	...
	// +optional
	DisableRedirect bool `json:"disableRedirect,omitempty" protobuf:"varint,6,opt,name=disableRedirect"`
	// requests controls how many parallel requests a given registry instance
	// will handle before queuing additional requests.
	// +optional
	Requests ImageRegistryConfigRequests `json:"requests,omitempty" protobuf:"bytes,7,opt,name=requests"`
	...
	// +optional
	DefaultRoute bool `json:"defaultRoute,omitempty" protobuf:"varint,8,opt,name=defaultRoute"`
...
type ImageRegistryConfigStorageS3 struct {
	...
	// +optional
	RegionEndpoint string `json:"regionEndpoint,omitempty" protobuf:"bytes,3,opt,name=regionEndpoint"`
	...
	// +optional
	KeyID string `json:"keyID,omitempty" protobuf:"bytes,5,opt,name=keyID"`
...
type ImageRegistryConfigStorageSwift struct {
	...
	// +optional
	AuthURL string `json:"authURL,omitempty" protobuf:"bytes,1,opt,name=authURL"`
	// authVersion specifies the OpenStack Auth's version.
	// +optional
	AuthVersion string `json:"authVersion,omitempty" protobuf:"bytes,2,opt,name=authVersion"`
	...
	// +optional
	DomainID string `json:"domainID,omitempty" protobuf:"bytes,5,opt,name=domainID"`
	...
	// +optional
	RegionName string `json:"regionName,omitempty" protobuf:"bytes,8,opt,name=regionName"`
}
...
type ImageRegistryConfigStoragePVC struct {
	...
	// +optional
	Claim string `json:"claim,omitempty" protobuf:"bytes,1,opt,name=claim"`
}
...
type ImageRegistryConfigRequests struct {
	...
	// +optional
	Read ImageRegistryConfigRequestsLimits `json:"read,omitempty" protobuf:"bytes,1,opt,name=read"`
	// write defines limits for image registry's writes.
	// +optional
	Write ImageRegistryConfigRequestsLimits `json:"write,omitempty" protobuf:"bytes,2,opt,name=write"`
}
...
type ImageRegistryConfigRequestsLimits struct {
	...
	// +optional
	MaxRunning int `json:"maxRunning,omitempty" protobuf:"varint,1,opt,name=maxRunning"`
	// maxInQueue sets the maximum queued api requests to the registry.
	// +optional
	MaxInQueue int `json:"maxInQueue,omitempty" protobuf:"varint,2,opt,name=maxInQueue"`
